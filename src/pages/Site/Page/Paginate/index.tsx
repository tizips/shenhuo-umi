...
import Editor from '@/pages/Site/Page/Editor';
...
const Paginate: React.FC = () => {
  ...
  const [search, setSearch] = useState<APISitePages.Search>({});
  const [editor, setEditor] = useState<APISitePages.Data | undefined>();
  const [load, setLoad] = useState(false);
  const [visible, setVisible] = useState<APISitePages.Visible>({});
  const [data, setData] = useState<APIData.Paginate<APISitePages.Data>>();
  ...
  const onDelete = (record: APISitePages.Data) => {
    if (data?.data) {
      const temp = { ...data };
      if (temp.data) {
      ...
      <Card
        ...
        extra={
          <Space size={[10, 10]}>
            ...
            <Access accessible={access.page('site.page.create')}>
              <Button
                onClick={() => {
                  setEditor(undefined);
      ...
      >
        ...
        >
          ...
          <Table.Column
            ...
            align="center"
            width={120}
            render={(record: APISitePages.Data) =>
          ...
          <Table.Column
            ...
            render={(record: APISitePages.Data) => (
              <>
                <Access accessible={access.page('site.page.update')}>
                  <Button
                    type="link"
                    onClick={() => {
                      setEditor(record);
                ...
                <Access accessible={access.page('site.page.delete')}>
                  <Popconfirm
                    title="确定要删除该数据?"
                    placement="leftTop"
