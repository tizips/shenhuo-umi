...
const PictureCard = (props: SortFormPictureCard.Props) => {
  ...
  const DraggableUploadListItem = ({
    originNode,
    file,
  }: SortFormPictureCard.DraggableUploadItemProps) => {
    ...
    return (
      ...
      >
        ...
        <div>{file.status === 'error' && isDragging ? originNode.props.children : originNode}</div>
      </div>
    );
  };
            ...
            {(!props.sources || props.sources.length <= 8) && (
              <div className={styles.button}>
                ...
                {(props.simple === undefined || !props.simple) && (
                  <div style={{ marginTop: 8 }}>{props.title ? props.title : '点击上传'}</div>
                )}
              </div>
            )}
          </Upload>
